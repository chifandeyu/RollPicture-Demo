...
RollPicture::RollPicture(QStringList picList, QSize picSize, int picCount,
	int speedTime, int delayTime,int space, QWidget *parent) : QWidget(parent),
	m_picList(picList), m_picSize(picSize), m_picCount(picCount),
	m_speedTime(speedTime), m_space(space), m_delayTime(delayTime)
{
	...
	connect(&timer, SIGNAL(timeout()), this, SLOT(changeItemPic()));
...
void RollPicture::initAnimationGroup()
{
	...
	connect(this->m_group, SIGNAL(finished()), this, SLOT(startTimer()));
...
void RollPicture::startTimer()
{
	timer.setInterval(m_delayTime);
	timer.start();
}
void RollPicture::changeItemPic()
{
	auto xx = qobject_cast<QObject*>(sender());
